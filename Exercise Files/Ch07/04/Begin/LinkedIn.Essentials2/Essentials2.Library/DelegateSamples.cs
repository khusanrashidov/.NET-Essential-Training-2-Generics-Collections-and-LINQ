...
namespace Essentials2.Library
{
    ...
    public static class DelegateSamples
    {
        ...
        public static void PassMeFunction(Func<long, double> work) // Func<in, out>  can accept in parameters and can return out parameters.
        {
            //work(); // an error
            double count = work(8);
            Console.WriteLine($"Func returned: {count}");
        }
        public static void PassMeAction(Action<string> work)
        {
            //work(); // an error
            work("Action");
        }
